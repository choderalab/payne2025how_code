...
def sp10 = make_plif_settings_map("1")   // :g format strips trailing zero: 1.0 → "1"

workflow PLIF_DATESPLIT_POSIT_0_75 { RUN_ANALYSIS(plif_result, [label: "datesplit_plif0.75", filename: sp75.datesplit]) }
workflow PLIF_DATESPLIT_POSIT_1_0  { RUN_ANALYSIS(plif_result, [label: "datesplit_plif1",    filename: sp10.datesplit]) }

// Scaffold splits for all three cutoffs — uses the same YAML files already
// generated by GENERATE_SETTINGS_PLIF_0_75 / GENERATE_SETTINGS_PLIF_1_0.
workflow PLIF_X_TO_X_POSIT_0_75       { RUN_ANALYSIS(plif_result, [label: "x_to_x_plif0.75",       filename: sp75.x_to_x]) }
workflow PLIF_X_TO_X_5_POSIT_0_75     { RUN_ANALYSIS(plif_result, [label: "x_to_x_5_plif0.75",     filename: sp75.x_to_x_5]) }
workflow PLIF_NOT_X_TO_X_POSIT_0_75   { RUN_ANALYSIS(plif_result, [label: "not_x_to_x_plif0.75",   filename: sp75.not_x_to_x]) }
workflow PLIF_NOT_X_TO_X_5_POSIT_0_75 { RUN_ANALYSIS(plif_result, [label: "not_x_to_x_5_plif0.75", filename: sp75.not_x_to_x_5]) }
workflow PLIF_X_TO_NOT_X_POSIT_0_75   { RUN_ANALYSIS(plif_result, [label: "x_to_not_x_plif0.75",   filename: sp75.x_to_not_x]) }

workflow PLIF_X_TO_X_POSIT_1_0        { RUN_ANALYSIS(plif_result, [label: "x_to_x_plif1",          filename: sp10.x_to_x]) }
workflow PLIF_X_TO_X_5_POSIT_1_0      { RUN_ANALYSIS(plif_result, [label: "x_to_x_5_plif1",        filename: sp10.x_to_x_5]) }
workflow PLIF_NOT_X_TO_X_POSIT_1_0    { RUN_ANALYSIS(plif_result, [label: "not_x_to_x_plif1",      filename: sp10.not_x_to_x]) }
...
// ── Top-4 scaffold filtered PLIF analysis ────────────────────────────────────
// The standard x_to_x YAML runs all 100+ scaffolds against themselves.
// These workflows restrict to only the top-4 scaffolds (IDs 0-3) via
// filter_plif_parquet.py before generating evaluators, so the self-docked and
// cross-scaffold analyses are confined to the four most prevalent series.
//
// Evaluators cover PLIF Tversky recall cutoffs 0.5, 0.75, 1.0 with RMSD and
// POSIT scorers and date + random reference splits (create_evaluators_filtered_plif.py).
//
// Run -entry PLIF_SELF_DOCKED_FILTERED or PLIF_NOT_TOP4_TO_TOP4_FILTERED,
// or use analyze_posit_plif_top4_filtered to run both together.

workflow PLIF_SELF_DOCKED_FILTERED {
    FILTER_PLIF_PARQUET(
        "self_docked",
        plif_result.docking_results_parquet,
        plif_result.docking_results_json,
    )

    CREATE_EVALUATORS_MODULAR(
        "self_docked_filtered_plif",
        "${projectDir}/scripts/create_evaluators_filtered_plif.py",
        FILTER_PLIF_PARQUET.output.filtered_parquet,
        FILTER_PLIF_PARQUET.output.filtered_json,
    )
    ...
    COMBINE_EVALUATIONS(
        "self_docked_filtered_plif",
        RUN_EVALUATORS.output.evaluator_results.flatten().collect(),
    )
}

workflow PLIF_NOT_TOP4_TO_TOP4_FILTERED {
    FILTER_PLIF_PARQUET(
        "not_top4_to_top4",
        plif_result.docking_results_parquet,
        plif_result.docking_results_json,
    ...
    CREATE_EVALUATORS_MODULAR(
        "not_top4_to_top4_filtered_plif",
        "${projectDir}/scripts/create_evaluators_filtered_plif.py",
        FILTER_PLIF_PARQUET.output.filtered_parquet,
        FILTER_PLIF_PARQUET.output.filtered_json,
    )
...
workflow analyze_posit_plif_top4_filtered {
    PLIF_SELF_DOCKED_FILTERED()
    PLIF_NOT_TOP4_TO_TOP4_FILTERED()
}
// ── end top-4 scaffold filtered PLIF analysis ────────────────────────────────

workflow analyze_all_new_plif {
    analyze_posit_plif_cutoffs()
    analyze_posit_plif_scorer()
    analyze_posit_plif_top4_filtered()
}
